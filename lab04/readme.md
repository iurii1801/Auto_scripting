...
Использование Jenkins предоставляет широкий набор преимуществ для DevOps-команд.
Прежде всего, Jenkins обеспечивает **непрерывную интеграцию (CI)** и **непрерывную доставку (CD)**, позволяя автоматически собирать, тестировать и развёртывать приложения при каждом изменении кода.
Он поддерживает множество плагинов, интеграций и языков программирования, что делает его универсальным инструментом для любых проектов.
Jenkins позволяет **сократить время ручных операций**, уменьшить количество ошибок, повысить надёжность сборок и обеспечить прозрачность всего процесса разработки.
Кроме того, благодаря своей архитектуре “controller–agent”, Jenkins легко масштабируется и поддерживает выполнение задач на разных машинах или контейнерах.

**2. Какие ещё типы агентов Jenkins существуют?**

Помимо **SSH-агентов**, которые используются для подключения через безопасный протокол SSH, в Jenkins существуют и другие типы агентов:

* **JNLP-агенты (Java Web Start)** — подключаются к контроллеру через JNLP-протокол и часто используются для Windows-сред.
* **Docker-агенты** — запускаются внутри контейнеров Docker, обеспечивая изолированные и легко воспроизводимые сборочные окружения.
* **Kubernetes-агенты** — создаются динамически в кластере Kubernetes для масштабирования CI/CD.
* **Static agents** — заранее настроенные узлы, которые постоянно подключены к контроллеру Jenkins.
* **Cloud agents** — временные экземпляры, создаваемые в облачных провайдерах (AWS, Azure, GCP) по мере необходимости.

Такое разнообразие типов агентов делает Jenkins гибкой системой, способной адаптироваться под любые инфраструктурные сценарии.

**3. С какими проблемами вы столкнулись при настройке Jenkins и как вы их решили?**

В процессе настройки Jenkins основными сложностями были:

* Ошибки при подключении SSH-агента к контроллеру — решались корректным созданием и добавлением **SSH-ключей** в раздел *Credentials* с типом **SSH Username with private key**.
* Проблема с загрузкой проекта из GitHub — устранена путём правильного указания ветки `lab04` и пути к скрипту `lab04/recipe-book/Jenkinsfile` в параметре **Script Path**.
* Отсутствие зависимостей в контейнере агента — решено добавлением установки **php-cli** и **composer** в `Dockerfile` агента.
  После устранения этих ошибок Jenkins успешно подключился к агенту, загрузил проект, выполнил сборку и прошёл все этапы конвейера без ошибок, завершившись статусом **SUCCESS**.
